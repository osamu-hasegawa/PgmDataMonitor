...
//            }
		...
		)
		{
			...
			for(int i = 0; i < list.Count; i++)
			{
				...
				int seikeiTotal = list[i].ryohinSuu + furyou;
				if(jj == 0)
				{
...
//					listView1.Items[i].UseItemStyleForSubItems = false;
        ...
        private void backup_timer_Tick(object sender, EventArgs e)
        {
			...
			{
				WriteMonthlyBackup(dateStr, currentSeihin, currentMachine);
			}
			else//2度目以降
			{
		...
		private void WriteMonthlyBackup(string dateStr, string seihin, string machine)
		{
			DateTime dt = DateTime.Now;//本日
			dateStr += "/20";//20日
        ...
        private void button5_Click(object sender, EventArgs e)
        {
			...
			{
				isSwitchType = true;
				SETDATA.machineType = 2;//シングルHS成型機
