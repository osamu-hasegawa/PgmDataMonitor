...
//            }
		...
		public bool DailyFileCopyToServer(string path)
		{
			try
			{
				...
				//フォルダ配下にCSVファイルがなければ保存
				string dailysign = dt.ToString("yyyyMMdd_HHmmss");
	            string dstFile = serverPath + "\\" + currentSeihin + "_" + currentMachine + "_" + dailysign + "_daily.csv";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
			...
			{
				isSwitchType = true;
				if(SETDATA.machineType == 0)//シングルLS成型機
				{
					SETDATA.machineType = 3;//マルチLS成型機
				}
				else if(SETDATA.machineType == 1)//シングルNQD成型機
				{
					SETDATA.machineType = 4;//マルチNQD成型機
				}
